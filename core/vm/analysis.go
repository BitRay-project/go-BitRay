...
// it's data (i.e. argument of PUSHxx).///maps byte to 1 bit
type bitvec []byte

func (bits *bitvec) set(pos uint64) {
	(*bits)[pos/8] |= 0x80 >> (pos % 8)           ////set pos^th bit value to be one
}
func (bits *bitvec) set8(pos uint64) {
	(*bits)[pos/8] |= 0xFF >> (pos % 8)           ////set first pos bit values to be zero
	(*bits)[pos/8+1] |= ^(0xFF >> (pos % 8))      ////set first pos bit values to be one
}
